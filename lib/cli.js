...
buildHandsontable({
  hotBranch: hotBranch,
  outputDir: 'test/public/lib/handsontable/disabled_plugins',
  includeTypes: [] // without plugins
}).then(function() {
  return buildHandsontable({
    outputDir: 'test/public/lib/handsontable/enabled_all_modules',
    includeTypes: ['all'] // all modules
  });
}).then(function(builder) {
  hotVersion = builder.hotPackage.version;

  if (builder.options.hotBranch && ['latest', 'master', 'link'].indexOf(builder.options.hotBranch) === -1) {
    hotVersion = builder.options.hotBranch.replace(/\//g, '-');
  }
