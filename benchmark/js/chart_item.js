...
(function(w, d) {
  ...
  var METRICS = {
    scriptTime: 'Script execution time in ms, including garbage collection and render (lower is better). ' +
        'To keep 60FPS execution time should not exceed 16.6ms.',
    pureScriptTime: 'Script execution time in ms, without garbage collection nor render (lower is better).',
    renderTime: 'Render time in and outside of script in ms (lower is better).',
    gcTime: 'Garbage collection time in and outside of script in ms (lower is better).',
  ...
  function parseTooltipMetrics(metric, value) {
    ...
    if (metric === 'scriptTime' || metric === 'pureScriptTime' || metric === 'renderTime') {
  ...
  function ChartItem(metric) {
    ...
    this.template.querySelector('.description').textContent = ' - ' + METRICS[metric];
  ...
  ChartItem.prototype._buildLegendTemplate = function(color, data) {
    ...
    template.querySelector('.description').textContent = data.selectedHotVersion + ', ' + data.selectedTest;
   ...
   * @private
   */
  ChartItem.prototype._generateFixedArray = function(count) {
    return new Array(count || 100).join(',').split(',');
  };
